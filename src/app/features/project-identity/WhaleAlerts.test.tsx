...
describe('WhaleAlerts', () => {
  ...
  it('renders nothing when there is genuinely no whale-sized activity right now (the common case)', async () => {
    fetchChainAssetTrades.mockResolvedValue({
      ...
      trades: [
        { chain: 'solana', contractAddress: 'Sol1', side: 'buy', amountUsd: 50, walletAddress: 'Wallet1111', id: 't1', occurredAt: null },
      ],
    });
    const { container } = render(<WhaleAlerts projectId={42} chainAssetId={7} whaleThreshold={5000} />);
  ...
  it('shows only trades clearing the whale threshold, using wallet shorthand and a factual buy/sell label', async () => {
    fetchChainAssetTrades.mockResolvedValue({
      asset: baseAsset, supported: true,
      trades: [
        { chain: 'solana', contractAddress: 'Sol1', side: 'buy', amountUsd: 50, walletAddress: 'SmallWallet111', id: 'small', occurredAt: null },
        {
          chain: 'solana', contractAddress: 'Sol1', side: 'buy', amountUsd: 12_345,
          walletAddress: 'BigWhaleWallet11111111111111abcd', id: 'whale', occurredAt: null,
        },
      ],
    });
  ...
  it('never uses hype/manipulative language (Bible §15 tone rule)', async () => {
    fetchChainAssetTrades.mockResolvedValue({
      asset: baseAsset, supported: true,
      trades: [
        { chain: 'solana', contractAddress: 'Sol1', side: 'buy', amountUsd: 50_000, walletAddress: 'Whale1111111111', id: 'whale', occurredAt: null },
      ],
    });
    render(<WhaleAlerts projectId={42} chainAssetId={7} whaleThreshold={5000} />);
  ...
  it('shows "Unknown wallet" rather than a fabricated address when walletAddress is null', async () => {
    fetchChainAssetTrades.mockResolvedValue({
      asset: baseAsset, supported: true,
      trades: [
        { chain: 'solana', contractAddress: 'Sol1', side: 'sell', amountUsd: 8000, walletAddress: null, id: 'whale', occurredAt: null },
      ],
    });
    render(<WhaleAlerts projectId={42} chainAssetId={7} whaleThreshold={5000} />);
  ...
  it('renders a transaction explorer link for each whale trade', async () => {
    fetchChainAssetTrades.mockResolvedValue({
      asset: baseAsset, supported: true,
      trades: [
        { chain: 'solana', contractAddress: 'Sol1', side: 'buy', amountUsd: 10_000, walletAddress: 'Whale1', id: 'whale-sig', occurredAt: null },
      ],
    });
    render(<WhaleAlerts projectId={42} chainAssetId={7} whaleThreshold={5000} />);
    expect(await screen.findByRole('link', { name: /view transaction/i })).toHaveAttribute(
      'href', 'https://solscan.io/tx/whale-sig',
    );
  });
});
